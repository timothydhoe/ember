"""
Generate alight.itermcolors (iTerm2 profile) from schemes/alight.yml.

Usage:
    uv run scripts/export_iterm2.py

Then in iTerm2: Settings -> Profiles -> Colors -> Color Presets... ->
Import..., select the generated file, then choose it from that same menu.
"""
...
OUTPUT_FILE = BASE_DIR / "terminal" / "iterm2" / "alight.itermcolors"
...
def main() -> bool:
    ...
    out["Background Color"] = hex_to_component_dict(palette["background"])
    out["Foreground Color"] = hex_to_component_dict(palette["foreground"])
    out["Cursor Color"] = hex_to_component_dict(palette["cursor"])
    out["Cursor Text Color"] = hex_to_component_dict(palette["background"])
    ...
    # self-verify: round-trip every color back to hex, confirm it matches alight.yml
